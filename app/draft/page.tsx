...
function PlayerSearch({
  ...
  label: string;
  positionFilter?: string[]; // e.g. ['C','L','R'] for forwards
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const filtered = positionFilter
          ? data.filter((p) => positionFilter.includes(p.position))
          : data;
        setResults(filtered);
        setOpen(true);
      } finally {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {open && !loading && results.length === 0 && query.length >= 2 && (
        <div className="absolute z-50 top-full mt-1 w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-slate-400 text-sm">
          No players found
        </div>
      )}
    </div>
...
export default function DraftPage() {
  ...
  const allPicked =
    forwards.every(Boolean) &&
    defensemen.every(Boolean) &&
    goalies.every(Boolean);
  ...
  function setSlot<T>(
    ...
    if ((val as unknown as PickedPlayer)?.id === 0) {
      ...
      const dupe = allPicked.find((p) => p.id === (val as unknown as PickedPlayer).id);
      if (dupe) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!allPicked) {
      setError("Pick all 12 players before submitting");
      return;
    }
    setSubmitting(true);
    setError("");
          ...
          <div className="space-y-2">
            {forwards.map((p, i) => (
              <PlayerSearch
                ...
                value={p}
                onSelect={(v) => setSlot(forwards, setForwards, i, v)}
              />
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {defensemen.map((p, i) => (
              <PlayerSearch
                key={i}
                label={`Defenseman ${i + 1}`}
                positionFilter={DEFENSE_POSITIONS}
                value={p}
                onSelect={(v) => setSlot(defensemen, setDefensemen, i, v)}
              />
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {goalies.map((p, i) => (
              <PlayerSearch
                key={i}
                label={`Goalie ${i + 1}`}
                positionFilter={GOALIE_POSITIONS}
                value={p}
                onSelect={(v) => setSlot(goalies, setGoalies, i, v)}
              />
            ))}
          </div>
        </div>
